...
abstract class DLEPlugins {
	...
	private static function loadplugins() {
		...
		self::$plugins = array();
		$files = $bad_plugins = $found_plugins = array();
		...
		$db->query( "SELECT id FROM " . PREFIX . "_plugins", false );
		while ( $row = $db->get_row() ) {
			$found_plugins[] = $row['id'];
		}
		$db->free();

		$db->query( "SELECT * FROM " . PREFIX . "_plugins_files WHERE active='1' ORDER BY id ASC", false );
		...
		while ( $row = $db->get_row() ) {
			...
			$files[$row['file']][] = array('id'=> $row['plugin_id'], 'action' => $row['action'], 'searchcode' => $row['searchcode'], 'replacecode' => $row['replacecode'] );
		}
		$db->free();
		...
		switch ( $mod['action'] ) {
			...
			case "replace":
				...
				if( preg_match($search, $content) ) {
					...
					$content = preg_replace($search, $mod['replacecode'],$content);
	...
	private static function prepare_search( $code ) {
		...
		foreach($codes as $code) {
			if( trim($code) ) {
				$safe_code[] = preg_replace( "/\s+/ms", "\s*", preg_quote( trim($code), '#') );
			}
		}
