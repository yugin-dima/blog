...
function build_js($js) {
	...
	if ($config['js_min']) {
		...
		$js_array[] = "<script type=\"text/javascript\" src=\"engine/classes/min/index.php?charset={$config['charset']}&amp;g=admin&amp;v=24\"></script>";

		if ( count($js) ) $js_array[] = "<script type=\"text/javascript\" src=\"engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;v=24\" defer></script>";
		...
		foreach ($js as $value) {
			...
			$js_array[] = "<script type=\"text/javascript\" src=\"{$value}?v=24\"{$defer}></script>";
...
function build_css($css) {
	...
	if ($config['js_min']) {
		...
		return "<link href=\"engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $css)."&amp;v=24\" rel=\"stylesheet\" type=\"text/css\">";

	} else {

		foreach ($css as $value) {
		
			$css_array[] = "<link href=\"{$value}?v=24\" rel=\"stylesheet\" type=\"text/css\">";
...
function check_referer( $current_path ) {
	...
	if( $ref['path'] AND $curr['path'] AND $ref['path'] == $curr['path'] ) {
